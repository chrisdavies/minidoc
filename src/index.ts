...
import { linkHoverPreview, LinkMenu, linkPlugin } from './link';
...
const plugins = [listPlugin, defaultPlugin, linkPlugin];

export function minidoc(doc: string): MinidocEditor {
  const editor = createCoreEditor({ doc, plugins });
  ...
  editor.toolbar && wrapper.append(editor.toolbar.root, editor.root);
